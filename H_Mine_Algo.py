...
def runHmine(input_file, output_file, min_support):
    ...
    itemsetBuffer = [
                        None] * 200  # This buffer variable is used to store the Items under recursion to generate the final frequent-itemset
    ...
    '''
       ...
       input: prefix of the variable want to find frequent, prefixlen is the length of the prefix before link with new element, 
               rowlist is the TID where the set check frequent.
       output: C - list of increase-in-size-1 item sets
       '''
    ...
    hmine(itemsetBuffer, 0,
          rowlist)  # Calling Hmine algorithm for first time using empty Buffer and 0 as prefixlength and initial value of rowlist Header.
    ...
    print(
        f'Data Mining completed using H-Mine algorithm for "{input_file}" dataset\nFrequent Itemsets stored in "{output_file}"')
